...
class Tasks::TimedTaskController < ApplicationController
  ...
  def update
    timed_task = current_user.timed_tasks.find(params[:id])
    update_timed_task = UpdateTimedTask.new(timed_task: timed_task, params: timed_task_params, user: current_user)
  ...
  def statistic_params_timed
    statistic_params_timed = @params.require(:statistic).permit(:timed_task, :cron)
