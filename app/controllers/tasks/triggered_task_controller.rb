...
class Tasks::TriggeredTaskController < ApplicationController
  ...
  def create
    home = current_user.homes.find(params[:home_id])
    create_triggered_task = CreateTriggeredTask.new(home: home, params: triggered_task_params, user: current_user)
  ...
  def update
    triggered_task = current_user.triggered_tasks.find(params[:id])
    update_triggered_task = UpdateTriggeredTask.new(triggered_task: triggered_task, params: triggered_task_params, user: current_user)
  ...
  def triggered_task_params
    triggered_task_params = params.require(:triggered_task).permit(:thing_id, :thing_to_compare_id, :scenario_id, :comparator, :display)
