...
class Thing < ApplicationRecord
  ...
  validates :name, :favorite, :type, :subtype, :connection_info, presence: true
  ...
  def status
    Curl.get(uri) do |http|
      ...
      http.headers["Authorization"] = room.home.token
    end
  end
  ...
  def uri
    room.home.tunnel + self.class.route + "?" + connection_params.to_query
  end
end
